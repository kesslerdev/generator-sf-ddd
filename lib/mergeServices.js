...
module.exports = (env, filePath, merged) => {
    ...
    parseString(env.fs.read(filePath), function (err, result) {
        ...
        if (key != undefined) {
            ...
            result.container.services = [{ service: newServices }];
        }
        ...
        if (!fileExists) {
            env.logger.snippet('you can add this serviceFile with line below in your bundle Extension:', 
                `$loader->load('` + path.basename(filePath) + `');`)
        }
